var/datum/planet/kaevis/planet_kaevis = null
...
/datum/planet/kaevis
	...
	current_time = new /datum/time/kaevis()
	expected_z_levels = list(
		Z_LEVEL_FIRST_PLANE,
		Z_LEVEL_SECOND_PLANE
	)

	planetary_wall_type = /turf/unsimulated/wall/planetary/kaevis

/datum/planet/kaevis/New()
	..()
	planet_kaevis = src
	weather_holder = new /datum/weather_holder/kaevis(src)

/datum/planet/kaevis/update_sun()
	..()
...
/datum/weather_holder/kaevis
	temperature = T0C
	allowed_weather_types = list(
		WEATHER_CLEAR		= new /datum/weather/kaevis/clear(),
		WEATHER_OVERCAST	= new /datum/weather/kaevis/overcast(),
		WEATHER_LIGHT_SNOW	= new /datum/weather/kaevis/light_snow(),
		WEATHER_SNOW		= new /datum/weather/kaevis/snow(),
		WEATHER_BLIZZARD	= new /datum/weather/kaevis/blizzard(),
		WEATHER_RAIN		= new /datum/weather/kaevis/rain(),
		WEATHER_STORM		= new /datum/weather/kaevis/storm(),
		WEATHER_HAIL		= new /datum/weather/kaevis/hail()
		)
...
datum/weather/kaevis
	name = "kaevis base"
	temp_high = 243.15 // -20c
	temp_low = 233.15  // -30c

/datum/weather/kaevis/clear
	name = "clear"
...
/datum/weather/kaevis/light_snow
	name = "light snow"
...
/datum/weather/kaevis/snow/process_effects()
...
/datum/weather/kaevis/blizzard/process_effects()
	for(var/turf/simulated/floor/outdoors/snow/S in outdoor_turfs)
		if(S.z in holder.our_planet.expected_z_levels)
			for(var/dir_checked in cardinal)
				var/turf/simulated/floor/T = get_step(S, dir_checked)
...
/datum/weather/kaevis/rain/process_effects()
...
/datum/weather/kaevis/hail/process_effects()
	for(var/mob/living/L in living_mob_list)
		if(L.z in holder.our_planet.expected_z_levels)
			var/turf/T = get_turf(L)
			if(!T.outdoors)
...
/datum/weather/kaevis/blood_moon
	name = "blood moon"
	light_modifier = 0.5
	light_color = "#FF0000"
	transition_chances = list(
